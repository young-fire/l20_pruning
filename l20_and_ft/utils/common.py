...
if args.arch == 'my_resnet_imagenet':
    args.job_dir = args.job_dir+'my_'+args.criterion+'_'+now
elif  args.arch == 'resnet_imagenet':
    args.job_dir = args.job_dir+'clr_'+args.criterion+'_'+now
...
def random_weight(weight,m,logger):
    ...
    if weight.dim() == 4:  #Convolution layer
        ...
        raise('The weight dim must be 4!')
    device = torch.device(f"cuda:{args.gpus[0]}") if torch.cuda.is_available() else 'cpu'
    indices = random.sample(range(0, W.size(0)-1), m)    #需要改的就是这里
    if args.graph_gpu:
        W = W.to(device)
