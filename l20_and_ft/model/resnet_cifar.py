...
class ResBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, mid_num, stride=1):
        super(ResBasicBlock, self).__init__()
        self.inplanes = inplanes
        self.planes = planes
        self.conv1 = conv3x3(inplanes, mid_num, stride)
        self.bn1 = nn.BatchNorm2d(mid_num)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(mid_num, planes)
...
class ResNet(nn.Module):
    def __init__(self, block, num_layers, layer_cfg=None, block_cfg=None, num_classes=10):
        ...
        self.layer_num = 0 
        self.inplanes = self.layer_cfg[self.layer_num]
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.inplanes)
        self.relu = nn.ReLU(inplace=True)

        self.layer1 = self._make_layer(block,  blocks=n, stride=1)
        self.layer2 = self._make_layer(block,  blocks=n, stride=2)
        self.layer3 = self._make_layer(block,  blocks=n, stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(self.layer_cfg[-1], num_classes)
    ...
    def _make_layer(self, block, blocks, stride):
        ...
        layers.append(block(self.layer_cfg[self.layer_num ], self.layer_cfg[self.layer_num+2],
                                 self.layer_cfg[self.layer_num+1], stride))
        self.layer_num += 2

        for i in range(1, blocks):
            layers.append(block(self.layer_cfg[self.layer_num ], self.layer_cfg[self.layer_num+2 ],
                                      self.layer_cfg[self.layer_num+1]))
            self.layer_num += 2
...
def resnet56(layer_cfg=None, block_cfg = None, **kwargs):
    if layer_cfg ==None:
        layer_cfg = [16]+[16]*18+[32]*18+[64]*18
